...
export default function App({ Component, pageProps }) {
  ...
  // useEffect를 쓰는 경우는 여러가지가 있는데 우리는 부수효과(side effect) 를 처리할때 사용함
  // 좀 더 쉽게 표현하면 DOM 관련 작업이나 이벤트 등록 등 렌더링 외의 작업을 처리할때 사용
  ...
  useEffect(() => {
    ...
    handleScroll();
    // 전역에 다시 호출해서 컴포넌트가 처음 로드될때 스크롤 위치 확인해서 헤더 class 붙여서 출력
    ...
    // 컴포넌트가 인마운트(화면에서 사라짐)될때 이벤트 리스너 제거
        ...
        <header className={isScrolled ? "header_area scroll" : "header_area"}>
          ...
          <div className="bottom">
            ...
            >
              ...
              {/* 위처럼 작성하면 필터링 함수 하드코딩 -> map 으로 사용해보자 */}
