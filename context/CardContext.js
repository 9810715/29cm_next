...
// useState : 리액트의 가장 기본적인 훅
// 훅이 뭔데? 리액트 함수형 컴포넌트 안에서 상태관리 사용할 수 있게 해주는 함수

const CardContext = createContext();

export const CardProvider = ({ children }) => {
  // children : 하단 return문에서 전달할 컴포넌트가 감싸고 있는 자식요소들
  const [cardData, setCardData] = useState([
    {
      ...
      explain:
        "세계적인 클래식 & 컨템포러리 가구, 조명 소품으로 구성된 인터네셔널 디자인 브랜드입니다",
    },
    {
      id: 2,
      imgSrc:
  ...
  const addCards = () => {
    if (cardIndex < additionalData.length) {
      ...
      // slice : slice() 메서드를 사용해서 cardIndex 부터 nextIndex + 1 값까지 요소를 추출하므로, 여기서는 3개의 데이터를 가져와 cardData 추가
      setCardIndex(nextIndex); // Index 를 3씩 증가 시킴

      // 무한 스크롤로 변경하면서 없앰
      // setTimeout(() => {
      //   // 세 요소가 추가된 후 하단으로 스크롤
  ...
  return (
    ...
    // CardContext.Provider 는 context 안의 값을 공급하는 컴포넌트
    // {children} 은 이 안에 감싸줄 컴포넌트를 의미
    // children 은 결국 return 문 안에서 <CardContext.Provider> 로 감싸줘서 전역상태(cardData 등) 를 하위 컴포넌트들이 사용할 수 있게함
  );
};

export const useCard = () => useContext(CardContext);
// 리액트 컨텍스트를 더 편하게 쓰기위한 커스텀 훅
// 커스텀 훅을 안쓰면 index.js 에 접근시킬때 useContext 에 import 도 시켜야 하고  useContext(CardContext);도 적어줘야함
