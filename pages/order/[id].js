...
export default function OrderDetail() {
  ...
  // const id = router.query.id; <- 이 구문을 간결하게 구조분해할당으로 쓴것
  // query 는 useRouter 속성
  // Next.js 에서 useRouter 사용하면 현재 페이지의 url 정보를 가져올 수 있음
  // query 는 현재 페이지 url 의 쿼리스트링 또는 동적 라우팅  파라미터를 담고 있음

  // 즉, query 객체에서 id 만 뽑아서 바로 id 라는 변수로 선언한다
  const { cardData } = useCard();
  // 전역에서 관리 중인 카드 목록 데이터 불러옴
  // cardData 배열에서 id 가 일치하는 상품을 찾아내주는 로직
      ...
      <div className="top_box">
        ...
        <div className="productbox">
          ...
          <div className="product productOrder">
            ...
            <div className="pakingInfo">
              <div className="pakingFlex">
            ...
            <div className="cartBox">
              <div className="countbox">
                {/* 과제 : 카운팅 ui 만들어보기 */}
                <button onClick={removeNumber} className="countui removebtn">
                  -
                </button>
                {/* <button
                onClick={() =>
                  (count > 0 ? setCountNumber(countNumber - 1) : null)
                }
                className="countui removebtn"
              >
                -
              </button> */}
                <button className="countui incre_number">{countNumber}</button>
                <button onClick={plusNumber} className="countui addBtn">
                  +
                </button>
                {/* <button
              ...
              <div className="buybtnbox">
                <a href="#none" className="buybtn cartbun">
                  장바구니 담기
                </a>
